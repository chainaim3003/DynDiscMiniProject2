...
```
task-data/
├── jupiterSellerAgent-info.json      # Agent AID and OOBI
├── Jupiter_Chief_Sales_Officer-info.json  # OOR holder AID
├── Jupiter_Knitting_Company-info.json     # LE AID and LEI
├── tommyBuyerAgent-info.json         # Agent AID and OOBI
├── Tommy_Chief_Procurement_Officer-info.json  # OOR holder AID
├── Tommy_Hilfiger_Europe-info.json   # LE AID and LEI
├── oor-credential-info.json          # OOR credential SAID
...
✓ LE info loaded: Jupiter_Knitting_Company
✓ OOR credential info loaded
✓ QVI info loaded
...
```json
{
  "extensions": {
    ...
    "vLEImetadata": {
      ...
      "verificationPath": [
        "GLEIF_ROOT → QVI",
        "QVI → JUPITER_KNITTING_COMPANY → Chief_Sales_Officer → jupiterSellerAgent"
      ],
      "status": "verified",
